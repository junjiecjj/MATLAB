...
for i_vec = 1 : length(vec)
    ...
    for i_run = 1 : max_runs
        ...
        for i_iter = 1:iter_all
            %****************************detector******************************
            %MPA detector
            [llr_detector_output, ~] = SCMA_MPAdetector_forIter(CB, y, h, sigma, iter_detector, col_valid, row_valid, bits, mset_double, prior_detector);
            %****************************detector******************************
            ...
            for j = 1:J
                ...
                for i = 1:frame_length
                    temp = llr_decoder_output((i-1)*bits_num+1:i*bits_num);
                    ...
                    for m = 1:M
                        ...
                        for b = 1:bits_num
                            prior_detector(j,:,m,i) = p(temp1(b)+1,b)*prior_detector(j,:,m,i); 
                        end
                    end
                    for i_dv = 1:d_v
                        prior_detector(j,i_dv,:,i) = prior_detector(j,i_dv,:,i)/sum(prior_detector(j,i_dv,:,i));%概率归一化
                    end
                end
            end
        end
