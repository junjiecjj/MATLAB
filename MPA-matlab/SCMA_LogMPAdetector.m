...
function decoded_symbols = SCMA_LogMPAdetector(CB, y, h, sigma, iterNum, col_valid, row_valid, mset_double)
    ...
    for iter = 1:iterNum
        ...
        for k = 1:K %遍历资源节点
            for idf = 1:d_f %遍历与当前资源节点相连的所有层节点
                iUser = row_valid(k,idf);%当前计算用户点
                layer = row_valid(k,[1:idf-1 idf+1:d_f]); %除当前用户节点外其余df-1个层节点序号
                for m = 1:M  %遍历当前用户节点的所有码字
                    for n = 0:M^(d_f-1)-1 %除当前用户外，其它用户码字的组合共有M^(d_f-1)种
        ...
        for j = 1:J %遍历所有用户节点
            for v = 1:d_v %遍历所有资源节点
                resource = col_valid([1:v-1 v+1:d_v],j);%除去当前资源节点
                for m = 1:M
                    for vm = 1:d_v-1
