...
function [llr_output, decoded_symbols] = SCMA_LogMPAdetector_forIter(CB, y, h, sigma, iterNum, col_valid, row_valid, bits, mset_double, prior)
            ...
            for idf = 1:d_f %遍历与当前资源节点相连的所有层节点
                ...
                for m = 1:M  %遍历当前层节点的所有码字
                    for n = 0:M^(d_f-1)-1 %除当前用户外，其它用户码字的组合共有M^(d_f-1)种，用四进制表示 15-33 从000到M-1 M-1 M-1。。。
                        ...
                        tmp = -(real(tmp)^2+imag(tmp)^2)/(2*sigma^2);%去掉常数项/(2*pi*sigma^2)
                        for c = 1:d_f-1 %加上V
