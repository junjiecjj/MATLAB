...
for kSubcarrier = 1:N
    for mSymbol = 1:M
        RxData(kSubcarrier, mSymbol) = sum(sqrt(SNR) * TxData(kSubcarrier, mSymbol).*exp(-1j * 2 * pi * fc * target_delay).*exp(1j * 2 * pi * mSymbol * Ts .* target_dop).*exp(-1j * 2 * pi * kSubcarrier .* target_delay * delta_f) ) + sqrt(1/2)* (randn() +1j * randn());
    end
end
