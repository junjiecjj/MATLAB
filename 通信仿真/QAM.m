% (ExampleQAMdemfilt.m)
% This program uses triangl.m to illustrate QAM modulation
...
xlabel('{\it t} (sec)'); ylabel('{\it s}_{\rm qam}({t})');
...
figure(4)
subplot(221);fd5=plot(freqm,abs(M2_fre));
axis(Frange); set(fd5,'Linewidth',1.5);
xlabel('{\it f} (Hz)'); ylabel('{\it M}({\it f})');
title('message 2 spectrum');
subplot(222);fd6=plot(freqs,abs(S_qam));
axis(Frange); set(fd6,'Linewidth',1.5);
xlabel('{\it f} (Hz)'); ylabel('{\it s}_{\rm QAM}({f})');
title('QAM spectrum magnitude');
