...
%
%  Output arguments: LLR - Log-Likelihood Ratio (size (log2(M)*V)xN)
...
parfor jj = 1:N
    ...
    for iter = 1:Nit
        ...
        for v = 1:V
            ind = find(F(:,v)==1);
            s1 = log(sum(exp(Igv(ind(1),v,:))));
            s2 = log(sum(exp(Igv(ind(2),v,:))));
            for m = 1:M
                ...
                Ivg(ind(1),v,m) = Igv(ind(2),v,m)-s2;
                Ivg(ind(2),v,m) = Igv(ind(1),v,m)-s1;
            end
        end
    end
